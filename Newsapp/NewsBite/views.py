...
def registerUser(request):														#function for registering input
	if request.method == 'POST':
		...
		if len(error) != 0:
			error = error[0]
			print(error)
			return render(request, 'NewsBite/registration_form.html', {'error': error})
		else:
		...
		return render(request, 'NewsBite/registration_form.html')
...
def index(request):																 #function defination for my startup page
	if request.method == 'POST':
		...
		return render(request, 'NewsBite/index.html')
...
def profile(request, username):
	...
	return render(request, 'NewsBite/profile.html', context)
